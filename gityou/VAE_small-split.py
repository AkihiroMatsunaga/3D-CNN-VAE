...
from tensorflow.keras.layers import Input, BatchNormalization, Conv3D, Dense, Flatten, Lambda, Reshape, Concatenate, Conv3DTranspose
...
def get_model():
    ...
    enc_nn1 = BatchNormalization()(
        Dense(
            units = 1000,
            kernel_initializer = 'glorot_normal',
            activation = mish)(Flatten()(enc_conv3)))
    enc_nn2 = BatchNormalization()(
        Dense(
            units = 2000,
            kernel_initializer = 'glorot_normal',
            activation = mish)(enc_nn1))
    enc_nn3 = BatchNormalization()(
        Dense(
            units = 1000,
            kernel_initializer = 'glorot_normal',
            activation = mish)(enc_nn2))
    
    enc_flt = Flatten()(enc_conv6)
    enc_fc0 = Concatenate()([enc_flt,enc_nn3])
